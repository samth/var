...
(require (except-in "lang.rkt" final-state?) "flat-check.rkt" "meta.rkt" "util.rkt" "annotate.rkt" "examples.rkt" 
...
(define-extended-language CESK* λcρ
  (K MT      
     (AP (clo ...) ((E ρ) ...) LAB a)
     (IF E E ρ a)
     (OP o (clo ...) (E ...) ρ LAB a)
     (LET x E ρ a)
     (BEG (E ρ) (E ρ) ... a)
     (CHK C ρ LAB LAB V_or-AE LAB a)  ;; V?     
     (CHK-CONS C ρ LAB LAB V_or-AE LAB V ρ a) ;; i hate the environment
     (CHK-OR V ρ (or/c FLAT HOC) ρ LAB LAB V_or-AE LAB a)) ;; (if [] (rem V FLAT) (HOC <= V))     
...
(define-metafunction CESK*
  widen : OP A -> A
  [(widen OP BLAME) BLAME]
  [(widen OP V) 
   V
   (side-condition (current-exact?))]
  [(widen OP V) (widen/n 10 OP V)])

(define-metafunction CESK*
  widen/n : natural o V -> V  
  [(widen/n natural o AV) AV]
  [(widen/n natural o (-- bool C ...)) (-- bool C ...)]
  [(widen/n natural first V) V]
  [(widen/n natural rest V) V]
  [(widen/n natural_0 o (-- natural C ...))
   (-- natural C ...)
   (side-condition (< (term natural) (term natural_0)))]
  [(widen/n natural_0 o (-- natural C ...)) (remember-contract (-- (pred exact-nonnegative-integer? Λ)) C ...)]
  [(widen/n natural o (-- string C ...)) 
   (-- string C ...)
   (side-condition (< (string-length (term string)) (term natural_0)))]
  [(widen/n natural o (-- string C ...)) (remember-contract (-- (pred string? Λ)) C ...)]
  [(widen/n 0 o (-- (cons V_1 V_2) C ...))   
   (remember-contract (-- (pred cons? Λ)) C ...)]
  [(widen/n natural o (-- (cons V_1 V_2) C ...))
   (-- (cons (widen/n ,(sub1 (term natural)) o V_1) 
             (widen/n ,(sub1 (term natural)) o V_2))
       C ...)]
  [(widen/n natural o (-- empty C ...)) (-- empty C ...)]
  [(widen/n natural o (-- L C ...)) (-- L C ...)]
  [(widen/n natural o blessed-L) blessed-L]
  [(widen/n natural o blessed-A) blessed-A]  
  [(widen/n natural o (-- PV C ...)) (remember-contract (-- (∧)) C ...)])
...
(define-metafunction CESK*
  ...
  [(plug E (OP o (V ρ_1) ... E_1 ... ρ LAB a))
   (@ o V ... E E_1 ... LAB)]
  [(plug E (AP ((V ρ_1) ...) ((E_1 ρ) ...) LAB a))
   (@ V ... E E_1 ... LAB)]
  [(plug E (LET x E_1 ρ a))
   (let x E E_1)]
  [(plug E (BEG (E_1 ρ) a)) ;; this is dead anyway
   (begin E E_1)]
  [(plug E (CHK C ρ LAB_1 LAB_2 V LAB_3 a))
   (C <= LAB_1 LAB_2 V LAB_3 E)])
...
(define (step* state)
  (match state    
    [(st (? AE? (cons '-- (list-no-order `(or/c ,C1 ...) C ...))) ρ σ K) 
     (S 'or-split
        (for/list ([c C1])
          (st (term (remember-contract (-- (∧) ,@C) ,c)) ρ σ K)))]
    
    [(st (? AE? (cons '-- (list-no-order `(rec/c ,x ,body) C ...))) ρ σ K)
     (S 'rec-unroll
        (list (st (term (remember-contract (-- (∧) ,@C) (unroll (rec/c ,x ,body))))
                  ρ σ K)))]
    
    [(st (V: V) ρ σ `(AP ,clo ((,E_0 ,ρ_0) ,rest ...) ,LAB ,a))
     (S 'ap-next
        (list (st E_0 ρ_0 σ `(AP (,@clo (,V ,ρ)) ,rest ,LAB ,a))))]
    [(st (V: V) ρ σ `(OP ,o ,clo (,E_0 ,rest ...) ,ρ_0 ,LAB ,a))
     (S 'op-next
        (list (st E_0 ρ_0 σ `(OP ,o (,@clo (,V ,ρ)) ,rest ,ρ_0 ,LAB ,a))))]
    
    [(st `(@ ,(? (redex-match CESK* op) op) ,E_0 ,E_1 ... ,LAB) ρ σ K)
     (S 'op-push
        (for/list ([a (alloc σ (list K))])
          (define σ_0 (extend-sto1 σ a K))
          (st E_0 ρ σ_0 `(OP ,op () ,E_1 ,ρ ,LAB ,a))))]
    
    [(st `(@ ,E_0 ,E_1 ... ,LAB) ρ σ K)
     (S 'ap-push
        (for/list ([a (alloc σ (list K))])
          (define σ_0 (extend-sto1 σ a K))
          (st E_0 ρ σ_0 `(AP () ,(map (λ (e) (list e ρ)) E_1) ,LAB ,a))))]
   ...
   ;; let
    ...
    [(st (V: V-proc) ρ σ `(AP () () ,LAB ,a))
     (match V-proc
       ...
       [`((--> (λ () ,C)) <= ,LAB_1 ,LAB_2 ,V_or-AE ,LAB_3 (addr ,a_f))
        (S 'nullary-blessed-β-dep
           (for/list ([clo (sto-lookup σ a_f)])
             (match-let* ([K `(CHK ,C ,ρ ,LAB_1 ,LAB_2 ,V_or-AE ,LAB_3 ,a)]
                          [`(,V_f ,ρ_f) clo]
                          [`(,a_k) (alloc σ (list K))]
                          [σ_1 (extend-sto1 σ a_k K)])
               (st V_f ρ_f σ_1 `(AP () () ,LAB ,a_k)))))]
       [`((--> ,C) <= ,LAB_1 ,LAB_2 ,V_or-AE ,LAB_3 (addr ,a_f))
        (S 'nullary-blessed-β
           (for/list ([clo (sto-lookup σ a_f)])
             (match-let* ([K `(CHK ,C ,ρ ,LAB_1 ,LAB_2 ,V_or-AE ,LAB_3 ,a)]
                          [`(,V_f ,ρ_f) clo]
       ...
       [_ 
        (choose [(and (term (∈ #t (δ (@ procedure? ,V-proc ★))))
                 ...
                 (S 'apply-abs0
                    (for/list ([K (sto-lookup σ a)])
                      (match-let* ([`(-- ,C ...) V-proc]
                        ...
                        (st (term (remember-contract (-- (∧)) ,@(for/list ([c C_0])
                ...
                [(and (term (∈ #t (δ (@ procedure? ,V-proc ★))))
                 ...
                 (S 'blame-arity
                    (for/list ([K (sto-lookup σ a)])
                      (st `(blame ,LAB  Λ ,V-proc λ ,V-proc) ρ σ K)))]                
    ...
    ;; unary+ application
    [(st (V: V) ρ σ `(AP ((,(V: V-proc) ,ρ_0) ,clo ...) () ,LAB ,a))
     (choose 
       [(and (term (∈ #t (δ (@ procedure? ,V-proc ★))))
           ...
           (S 'β
              (for/list ([K (sto-lookup σ a)])
                (st E 
                    ...
                    K)))]          
          [`((,C_0 ... --> (λ (,x ...) ,C_1)) <= ,LAB_1 ,LAB_2 ,V_or-AE ,LAB_3 (addr ,a_f))
           (match-let* ([a_1 (alloc σ x)]
                        (ρ_3 (extend-env ρ x a_1))
                        [K `(CHK ,C_1 ,ρ_3 ,LAB_1 ,LAB_2 ,V_or-AE ,LAB_3 ,a)]
             ...
             (S 'unary+-blessed-β-dep
                (for/list ([clo_1 (sto-lookup σ a_f)])
                  ...
                  (st V_f ρ_f σ_1 K2))))]
          [`((,C_0 ... --> ,C_1) <= ,LAB_1 ,LAB_2 ,V_or-AE ,LAB_3 (addr ,a_f))
           ;(printf "got here \n")
           (match-let* ([K `(CHK ,C_1 ,ρ ,LAB_1 ,LAB_2 ,V_or-AE ,LAB_3 ,a)]
                        [(list a_k) (alloc σ (list K))]
                        [σ_1 (extend-sto1 σ a_k K)]
                        [K2 `(AP ()
                                 ,(for/list ([clo_1 (cons (list V ρ) clo)]
                                    ...
                                    (match-let ([(list V_2 ρ_2) clo_1])
                                      `((,C_0* <= ,LAB_2 ,LAB_1 ,V_2 ,LAB_3 ,V_2) ,ρ_2)))
                                 ,LAB ,a_k)])
             (S 'unary+-blessed-β
                (for/list ([clo_1 (sto-lookup σ a_f)])
                  (match-define (list V_f ρ_f) clo_1)
              ...
              (match-let ([dom-con (term (domain-contracts ,C))])
                (for/list ([clo_1 `((,V ,ρ) . ,clo)]
                  ...
                  (match-let* ([`(,U ,ρ_2) clo_1]
                               ...
                               [E_result (term (remember-contract (-- (∧)) ,@C_0))])                    
       ...
       [(and (term (∈ #t (δ (@ procedure? ,V-proc ★))))
        ...
        (S 'apply-abs-no-arity
           (for/list ([clo* (cons (list V ρ) clo)])
             ...
             (st (term (amb (-- 0) (demonic* (∧) ,U))) ρ_2 σ (term (BEG ((-- (∧)) #hash()) ,a)))))]
       [(and (term (∈ #t (δ (@ procedure? ,V-proc ★))))
             (not (equal? (add1 (length clo))
                          (term (arity ,V-proc)))))
        (S 'blame-arity
           (for/list ([K (sto-lookup σ a)])
             (st `(blame ,LAB  Λ ,V-proc λ ,V-proc) ρ_0 σ K)))]
   ...
   ;; FIXME broken delta rule for things producing closures.
    [(st (V: V) ρ σ `(OP ,op ,(list (list V_0 ρ_0) ...) () ,ρ_1 ,LAB ,a))
     (S 'δ
        (for*/list ([K (sto-lookup σ a)]
                    [A (term (δ ,`(@ ,op ,@V_0 ,V ,LAB)))])
          (st (term (widen ,op ,A)) '#hash() σ K)))]
    ...
    [(st (V: V) ρ σ `(CHK (,C ... -> ,result) ,ρ_1 ,LAB_1 ,LAB_2 ,V_or-AE ,LAB_3 ,a))
     (choose
      [(term (∈ #t (δ (@ procedure? ,V ★))))
       (S 'chk-fun-pass
        (match-let* ([(list a_new) (alloc σ (list V))]
          ...
          (for/list ([K (sto-lookup σ a)])
            (st `((,@C --> ,result) <= ,LAB_1 ,LAB_2 ,V_or-AE ,LAB_3 (addr ,a_new)) ρ_1 σ_1 K))))]
      [(term (∈ #f (δ (@ procedure? ,V ★))))
       (S 'chk-fun-fail-fail
          (for/list ([K (sto-lookup σ a)])
            (st `(blame ,LAB_1 ,LAB_3 ,V_or-AE (,@C -> ,result) ,V) ρ σ K)))])]
    
    
    
    [(st (V: V) ρ σ `(CHK ,(? (redex-match CESK* FLAT) FLAT) ,ρ_1 ,LAB_1 ,LAB_2 ,V_or-AE ,LAB_3 ,a))
     (match-let* ([K
                   `(IF ,(term (remember-contract ,V (try-close-contract ,FLAT ,ρ_1 ,σ)))
                       (blame ,LAB_1 ,LAB_3 ,V_or-AE ,FLAT ,V)
                       ,ρ ,a)]
                  [(list a_k) (alloc σ (list K))]
    ...
    [(st (V: V) ρ σ `(CHK (rec/c ,X ,HOC) ,ρ_1 ,LAB_1 ,LAB_2 ,V_or-AE ,LAB_3 ,a))
     (S 'chk-unroll
        (list (st V ρ σ `(CHK ,(term (unroll (rec/c ,X ,HOC))) ,ρ_1 ,LAB_1 ,LAB_2 ,V_or-AE ,LAB_3 ,a))))]
    
    [(st (V: V) ρ σ `(CHK (or/c ,FLAT ,HOC) ,ρ_1 ,LAB_1 ,LAB_2 ,V_or-AE ,LAB_3 ,a))
     (match-let* ([K `(CHK-OR ,V ,ρ (or/c ,FLAT ,HOC) ,ρ_1 ,LAB_1 ,LAB_2 ,V_or-AE ,LAB_3 ,a)]                   
                  [(list a_k) (alloc σ (list K))]
                  [σ_new (extend-sto1 σ a_k K)])
       (S 'check-or-pass
        (list
   ...
   [(st (V: V) ρ_0 σ `(CHK-OR ,U ,ρ (or/c ,FLAT ,HOC) ,ρ_1 ,LAB_1 ,LAB_2 ,V_or-AE ,LAB_3 ,a))
    (choose 
     [(term (∈ #t (δ (@ false? ,V Λ))))
      (S 'check-or-false (list (st U ρ σ `(CHK ,HOC ,ρ_1 ,LAB_1 ,LAB_2 ,V_or-AE ,LAB_3 ,a))))]
     [(term (∈ #f (δ (@ false? ,V Λ))))
      (S 'check-or-true
         (for/list ([K (sto-lookup σ a)])
           (st (term (remember-contract ,U (try-close-contract ,FLAT ,ρ_1 ,σ))) ρ σ K)))])]
    
    [(st (V: V) ρ σ `(CHK (and/c ,C_0 ,C_1) ,ρ_1 ,LAB_1 ,LAB_2 ,V_or-AE ,LAB_3 ,a))
     (match-let* ([K `(CHK ,C_1 ,ρ_1 ,LAB_1 ,LAB_2 ,V_or-AE ,LAB_3 ,a)]                   
                  [(list a_k) (alloc σ (list K))]
                  [σ_1 (extend-sto1 σ a_k K)])
       (S 'check-and-push 
          (list (st V ρ σ_1 `(CHK ,C_0 ,ρ_1 ,LAB_1 ,LAB_2 ,V_or-AE ,LAB_3 ,a_k)))))]
   
    [(st `(-- (cons ,V_0 ,V_1) ,C ...) ρ σ `(CHK (cons/c ,C_0 ,C_1) ,ρ_1 ,LAB_1 ,LAB_2 ,V_or-AE ,LAB_3 ,a))
     (match-let* ([K `(CHK-CONS ,C_1 ,ρ_1 ,LAB_1 ,LAB_2 ,V_or-AE ,LAB_3 ,V_1 ,ρ ,a)]                   
                  [(list a_k) (alloc σ (list K))]
                  [σ_new (extend-sto1 σ a_k K)])
       (S 'check-cons-pass-first 
          (list (st V_0 ρ σ_new `(CHK ,C_0 ,ρ_1 ,LAB_1 ,LAB_2 ,V_or-AE ,LAB_3 ,a_k)))))]
   
    [(st (V: V) ρ σ `(CHK-CONS ,C_1 ,ρ_1 ,LAB_1 ,LAB_2 ,V_or-AE ,LAB_3 ,V_1 ,ρ_2 ,a))
     (match-let* ([K `(OP cons ((,V ,ρ)) () #hash() Λ ,a)]
                  [(list a_k) (alloc σ (list K))]
                  [σ_new (extend-sto1 σ a_k K)])
     (S 'check-cons-pass-rest
        (list (st V_1 ρ_2 σ_new `(CHK ,C_1 ,ρ_1 ,LAB_1 ,LAB_2 ,V_or-AE ,LAB_3 ,a_k)))))]
    
    
    [(st `(,C <= ,LAB_1 ,LAB_2 ,V_or-AE ,LAB_3 ,(? (redex-match CESK* E) E)) ρ σ K)
     (match-let* ([`(,a) (alloc σ (list K))]
                  [σ_0 (extend-sto1 σ a K)])       
       (S 'chk-push 
          (list (st E ρ σ_0
                    `(CHK ,C ,ρ ,LAB_1 ,LAB_2 ,V_or-AE ,LAB_3 ,a)))))]        
  ...
  (match s 
    ...
    [(st `(,f_1 ^ ,LAB ,f) ρ σ K)
     ...
     (cond [(redex-match CESK* bullet V)
            (S '∆-opaque 
               (list
                (st (term (,C <= ,f ,LAB (-- ,C) ,f_1 (remember-contract (-- (∧)) ,C)))
                    ρ σ K)))]
           [else
            (S '∆-other
               (list (st (term (,C <= ,f ,LAB (-- ,V) ,f_1 (-- ,V)))
                         ρ σ K)))])]
...
(define-metafunction CESK*
  ...
  [(live-loc-E (any_C <= LAB_1 LAB_2 any_V LAB_2 any_E)) (live-loc-E (any_C any_E))]
  [(live-loc-E (CHK any_C any_ρ LAB_1 LAB_2 any_V LAB_2 any_E any_ρ2 any_a)) (live-loc-E (any_C any_ρ any_E any_ρ2 any_a))]
  [(live-loc-E (blame LAB_1 LAB_2 any_V any_C any_V2)) (live-loc-E (any_C any_V2))]
  [(live-loc-E (addr a)) (a)]
  [(live-loc-E (any ...))
   (a ... ...)
...
(define-metafunction CESK*
  ...
  [(live-loc-K (BEG (E ρ) ... a))
   (a a_0 ... ...)
   (where ((a_0 ...) ...) ((live-loc-clo (E ρ)) ...))]
  ;; Probably want V_or-AE to be a closure and traverse it as well.
  [(live-loc-K (CHK C ρ LAB_0 LAB_1 V_or-AE LAB_2 a))
   (a a_0 ... a_1 ...)
   (where (a_0 ...) (live-loc-E C))
   (where (a_1 ...) (live-loc-env (restrict ρ (fv/C C))))]
  [(live-loc-K (CHK-OR V ρ C ρ_2 LAB_1 LAB_2 V_or-AE LAB_3 a))
   (a a_0 ... a_1 ... a_2 ...)
   (where (a_0 ...) (live-loc-E C))
   (where (a_2 ...) (live-loc-clo (V ρ)))
   (where (a_1 ...) (live-loc-env (restrict ρ_2 (fv/C C))))]
  [(live-loc-K (CHK-CONS C ρ LAB_0 LAB_1 V_or-AE LAB_2 V ρ_2 a))
   (a a_0 ... a_1 ... a_2 ...)
   (where (a_0 ...) (live-loc-E C))
...
(define ((colorize Ms) x node)
  ...
  (cond [(redex-match CESK* (V any any_1 MT) x*) "green"]
        [(redex-match CESK* (B any any_1 MT) x*)
         (redex-let CESK*
                    ([(blame LAB LAB_0 V any V_0) (car x*)])
...
(test
 ...
 (test-->>c step-gc-R (term (@ procedure? (-- ((∧) -> (∧))) †)) (term (-- #t)))
 ...
 (test-->>c step-gc-R (term (@ procedure? #f †)) (term (-- #f)))
 (test-->>c step-gc-R (term (@ cons 1 2 †)) (term (-- (cons (-- 1) (-- 2)))))
 (test-->>c step-gc-R (term (@ ((and/c (∧) ((∧) -> (∧))) <= f g (-- 0) f (-- (λ (x) x)))
                               5 h))
            (term (-- 5))))
...
(test
 ...
 (test-->>c step-gc-R (term (@ (-- (-> (pred exact-nonnegative-integer? Λ))) f)) (term (-- (pred exact-nonnegative-integer? Λ))))
 (test-->>c step-gc-R (term ((pred exact-nonnegative-integer? Λ) <= f g (-- 0) f (-- 5))) (term (-- 5)))
 (test-->>c step-gc-R 
            (term ((pred exact-nonnegative-integer? Λ) <= f g (-- 0) f (-- (λ (x) x))))
            (term (blame f f (-- 0) (pred exact-nonnegative-integer? Λ) (-- (λ (x) x)))))
 (test-->>c step-gc-R 
            (term ((pred exact-nonnegative-integer? Λ) <= f g (-- 0) f (-- #t))) 
            (term (blame f f (-- 0) (pred exact-nonnegative-integer? Λ) (-- #t))))
 (test-->>c step-gc-R 
            (term (((∧)  -> (∧)) <= f g (-- 0) f (-- (λ (x) x))))
            ;; kind of a giant hack
            (term (((∧) --> (∧)) <= f g (-- 0) f (addr ,(car (alloc (hash '(loc 0) (set '((-- 0) #hash()))) `((-- (λ (x) x)))))))))
 (test-->>c step-gc-R 
            (term (((∧)  -> (∧)) <= f g (-- 0) f (-- 5)))
            (term (blame f f (-- 0) ((∧) -> (∧)) (-- 5))))
 (test-->>c step-gc-R
            (term ((pred (λ (x) 0) LAB) <= f g (-- 0) f (-- 5)))
            (term (-- 5 (pred (λ (x) 0) LAB))))
 (test-->>c step-gc-R
            (term (begin 3 4))
            (term (-- 4)))
 (test-->>c step-gc-R
            (term ((and/c (pred exact-nonnegative-integer? Λ) (pred empty? Λ)) <= f g (-- 0) f (-- #t)))
            (term (blame f f (-- 0) (and/c (pred exact-nonnegative-integer? Λ) (pred empty? Λ)) (-- #t)))))
...
(test 
 ...
 (test-->>p (term (annotator [(simple-module p ((or/c exact-nonnegative-integer? exact-nonnegative-integer?) . -> . exact-nonnegative-integer?) •)
                        ...
                        (p 1)]))
            (term (-- (pred exact-nonnegative-integer? p)))) 
 (test-->>p (term [(require) ((pred string? Λ) <= |†| rsa (-- "Plain") rsa (-- "Plain"))])
            (term (-- "Plain"))) 
 #; ;; unbound module var
